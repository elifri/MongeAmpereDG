  ...
  setting_(opticalSetting),configFileEllipsoid(configFileEllipsoid)
{
  ...
  this->op = std::make_shared<OperatorType>(*this, setting_);
   ...
   const double integralLightOut = get_refl_operator().get_actual_g().integrateOriginal();
   const double integralLightIn = get_refl_operator().get_actual_f().integrateOriginal();
