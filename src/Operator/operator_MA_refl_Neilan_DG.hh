...
public:
    ...
    inline static Solver_config::value_type omega(
            Solver_config::SpaceType2d x) {
...
//		adouble a_tilde_value = 0;
    ...
    void assemble_cell_term(const LocalView& localView,
        ...
        for (size_t pt = 0; pt < quad.size(); pt++) {
            ...
            const double integrationElement = geometry.integrationElement(
                    quadPos);
            ...
            assemble_functionValues_u(localFiniteElementu, quadPos,
                    referenceFunctionValues, x_adolc.segment(0, size_u),
                    u_value);
            ...
            assemble_hessians_hessu(localFiniteElementu, jacobian, quadPos,
                    Hessians, x_adolc.segment(0, size_u), Hessu);

            //the shape function values of hessian ansatz functions and assemble u_DH
            std::vector<HessianType> referenceFunctionValuesHessian(
                    size_u_DH);
            ...
            assemble_functionValues_u(localFiniteElementuDH, quadPos,
                    ...
                    x_adolc.segment(size_u, size_u_DH), uDH);
            ...
            Solver_config::SpaceType3d X = { x_value[0], x_value[1], omega(
                    x_value) };

            adouble a_tilde_value = a_tilde(u_value, gradu, x_value);
            adouble b_tilde = gradu * gradu + sqr(u_value)
                    - sqr(gradu * x_value);
...
//		pertubed_matrix += a_tilde*(z_3/2/t/x_3)*N;
            ...
            adouble PDE_rhs = a_tilde_value * a_tilde_value * a_tilde_value
                    * f_value / (4.0 * b_tilde * omega(x_value) * g_value);

            //calculate system for first test functions

            for (size_t j = 0; j < size_u; j++) // loop over test fcts
                    {
                v_adolc(j) += (PDE_rhs - uDH.determinant())
...
//				std::cout << "det(u)-f=" << uDH.determinant()<<"-"<< f <<"="<< uDH.determinant()-f<< std::endl;
            ...
            for (size_t j = 0; j < size_u_DH; j++) // loop over test fcts
            {
                ...
                case 1: row = 1; col = 0; break;
                case 2: row = 0; col = 1; break;
                case 3: row = 1; col = 1; break;
                }

                v_adolc(size_u + j) += uDH[row][col]*referenceFunctionValuesHessian[j] * quad[pt].weight()
                        * integrationElement;
                v_adolc(size_u + j) -= Hessu[row][col]*
                        referenceFunctionValuesHessian[j] * quad[pt].weight()
                        * integrationElement;
            }
        }
        for (int i = 0; i < size; i++)
...
//			  << "numer of buffer size " << stats[4] << std::endl;
    ...
    void assemble_inner_face_term(const IntersectionType& intersection,
            ...
            const VectorType &x,
            const MixedElement<LocalElement0, LocalElement1> &localFiniteElementn,
            const VectorType &xn, VectorType& v, VectorType& vn,
            int tag) const {
        ...
        const int order = std::max(1,
                std::max(2 * ((int) localFiniteElement.order()),
        ...
        const FieldVector<double, dim - 1>& face_center =
                ReferenceElements<double, dim - 1>::general(
                        intersection.geometry().type()).position(0, 0);
        ...
        double penalty_weight = Solver_config::sigma
                * (Solver_config::degree * Solver_config::degree)
                / std::pow(intersection.geometry().volume(),
                        Solver_config::beta);
        ...
        Eigen::Matrix<adouble, Eigen::Dynamic, 1> xn_adolc(
                localFiniteElement.size());
        Eigen::Matrix<adouble, Eigen::Dynamic, 1> v_adolc(
                localFiniteElement.size());
        Eigen::Matrix<adouble, Eigen::Dynamic, 1> vn_adolc(
                localFiniteElement.size());
        ...
        for (size_t pt = 0; pt < quad.size(); pt++) {
            ...
            const FieldVector<double, dim> &quadPosn =
                    intersection.geometryInOutside().global(
                            quad[pt].position());

            const auto& jacobian =
                    intersection.inside()->geometry().jacobianInverseTransposed(
                            quadPos);
...
//		std::cout << std::endl;
            adouble un_value = 0;
            assemble_functionValues_u(localFiniteElement(u()), quadPosn,
                    referenceFunctionValuesn, xn_adolc.segment(0, size_u),
                    un_value);

            // The gradients of the shape functions on the reference element
            std::vector<typename LocalElement0::JacobianType> gradients(size_u);
            FieldVector<adouble, Solver_config::dim> gradu;
            assemble_gradients_gradu(localFiniteElement(u()), jacobian, quadPos,
                    gradients, x_adolc.segment(0, size_u), gradu);
            std::vector<typename LocalElement0::JacobianType> gradientsn(
                    size_u);
            FieldVector<adouble, Solver_config::dim> gradun;
            assemble_gradients_gradu(localFiniteElement(u()), jacobian,
                    quadPosn, gradientsn, xn_adolc.segment(0, size_u), gradun);
            ...
            std::vector<typename LocalElement1::TensorRangeType> referenceFunctionValuesHessian(
                    size_u_DH);
            localFiniteElement(u_DH()).localBasis().evaluateFunction(quadPos,
                    referenceFunctionValuesHessian);
            std::vector<typename LocalElement1::TensorRangeType> referenceFunctionValuesHessiann(
                    size_u_DH);
            localFiniteElement(u_DH()).localBasis().evaluateFunction(quadPosn,
            ...
            auto integrationElement =
                    intersection.geometry().integrationElement(
                            quad[pt].position());
            ...
            for (unsigned int j = 0; j < size_u; j++) {
                ...
                v_adolc(j) += penalty_weight * u_jump
                ...
                //neighbour parts
                // NIPG / SIPG penalty term: sigma/|gamma|^beta * [u]*[v]
                vn_adolc(j) += penalty_weight * u_jump
                        * (-referenceFunctionValuesn[j]) * factor;
                // gradient penalty
...
//	    std::cout << "phi " << phi_value << " atilde " << a_tilde_value << " T " << T_value[0] << " " << T_value[1] << std::endl;
     ...
     * @param v					 local residual (to be returned)
     */
    template<class Element, class LocalElement0, class LocalElement1,
            class VectorType, class MatrixType>
    void assemble_cell_Jacobian(const Element& element,
            const MixedElement<LocalElement0, LocalElement1> &localFiniteElement,
...
//TODO any better way to initialise matrix?
    ...
    void assemble_inner_face_Jacobian(const Intersection& intersection,
            ...
            const VectorType &x,
            const MixedElement<LocalElement0, LocalElement1> &localFiniteElementn,
            const VectorType &xn, MatrixType& m_m, MatrixType& mn_m,
