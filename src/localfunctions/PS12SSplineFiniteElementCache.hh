...
#ifndef SRC_LOCALFUNCTIONS_PS12SSPLINEFINITEELEMENTCACHE_HH_
#define SRC_LOCALFUNCTIONS_PS12SSPLINEFINITEELEMENTCACHE_HH_
...
//      return temp;
   ...
   */
  template<class GV, class D, class R, class SparseMatrixType>
...
//    typedef typename FixedOrderLocalBasisTraits<typename P0LocalFiniteElement<D,R,dim>::Traits::LocalBasisType::Traits,diffOrder>::Traits T;
    typedef PS12SSplineFiniteElement<Geometry, D, R, SparseMatrixType> FE;
    typedef typename std::map<Geometry,SparseMatrixType*, GeometryCompare<Geometry>> FEMap;


    static const int dofPerCell_ = 12;

    void create_hermite_interpolation_matrix(const Element &e, SparseMatrixType& A) const
    {
  ...
  public:
    ...
    std::shared_ptr<const FiniteElementType> get(const Element& element) const
    {
//      std::cout << "geo center " << geo.center() << std::endl;
      ...
      if (it==cache_.end())
      {
        SparseMatrixType* A = new SparseMatrixType(dofPerCell_, dofPerCell_);

        create_hermite_interpolation_matrix(element, *A);

        cache_[geo] = A;
//        std::cout << "inserted geo " << geo.center()  << " with center " << fe->geo_.center() << std::endl;

        return std::shared_ptr<const FE> (new const FE(geo, *A));
      }

      return std::shared_ptr<const FE>(new const FE(geo, *(it->second)));
    }
