...
class MAMixedLocalIndexSet<GV, 2, 2> {
...
public:
    ...
    typedef MAMixedBasisLocalView<GV, 2, 2> LocalView;

    /** \brief Type used for global numbering of the basis vectors */
    typedef std::array<size_type, 2> MultiIndex;

    MAMixedLocalIndexSet(const MAMixedIndexSet<GV, 2, 2> & indexSet) :
    ...
    void bind(const MAMixedBasisLocalView<GV, 2, 2>& localView) {
...
private:
    ...
    MAMixedIndexSet<GV, 2, 2> indexSet_;
    typename PQ2NodalBasis<GV>::IndexSet::LocalIndexSet uLocalIndexSet_;
    typename PQ2NodalBasis<GV>::IndexSet::LocalIndexSet uDHLocalIndexSet_;
};
...
template<typename GV>
  class MAMixedIndexSet<GV, 2, 2> {
    static const int dim = GV::dimension;
    static const int nDH = dim*dim;

    /** \brief The global FE basis that this is a view on */
    typedef MAMixedBasis<GV, 2, 2> GlobalBasis;
...
public:
    ...
    typedef MAMixedLocalIndexSet<GV, 2, 2> LocalIndexSet;
...
private:
    ...
    typename PQ2NodalBasis<GV>::IndexSet uDHIndexSet_;
};
...
template<typename GV>
  class MAMixedBasis<GV, 2, 2>: public GridViewFunctionSpaceBasis<GV,
          MAMixedBasisLocalView<GV, 2, 2>,
          MAMixedIndexSet<GV, 2, 2>, std::array<std::size_t, 2> > {
    static const int dim = GV::dimension;
    static const int nDH = dim*dim;
...
public:
    ...
    typedef PQ2NodalBasis<GV> BasisuDH;
    ...
    MAMixedIndexSet<GV, 2, 2> indexSet() const {
        return MAMixedIndexSet<GV, 2, 2>(gridView());
    }
...
private:
    friend MAMixedIndexSet<GV, 2, 2> ;
    friend MAMixedBasisLocalView<GV, 2, 2> ;

    PQ2NodalBasis<GV> unodalbasis_;
    PQ2NodalBasis<GV> uDHnodalbasis_;
};
...
template<typename GV>
  class MAMixedBasisLocalView<GV, 2, 2> {
    static const int dim = GV::dimension;
    static const int nDH = dim*dim;
...
public:
     ...
     */
    typedef MAMixedBasisTree<GV, 2, 2> Tree;
...
protected:
    friend MAMixedLocalIndexSet<GV, 2, 2> ;

    const GlobalBasis* globalBasis_;
    PQ2NodalBasisLocalView<GV> ulocalView_;
    PQ2NodalBasisLocalView<GV> uDHlocalView_;
    MAMixedHessianTree<GV, 2, 2> uDHTree_;
    Tree tree_;
};

template<typename GV, int deg, int degHess>
class MAMixedHessianTree: public TypeTree::PowerNode<PQ2NodalBasisLeafNode<GV>,
        GV::dimension*GV::dimension> {};

template<typename GV>
  class MAMixedHessianTree<GV, 2, 2>: public TypeTree::PowerNode<PQ2NodalBasisLeafNode<GV>,
          GV::dimension*GV::dimension> {

    friend class MAMixedBasisLocalView<GV, 2, 2>;

    MAMixedHessianTree(PQ2NodalBasisLeafNode<GV> & leafNode) :
            TypeTree::PowerNode<PQ2NodalBasisLeafNode<GV>, GV::dimension*GV::dimension>(
...
template<typename GV>
class MAMixedBasisTree<GV, 2, 2>: public TypeTree::CompositeNode<PQ2NodalBasisLeafNode<GV>,MAMixedHessianTree<GV, 2, 2>> {
    friend MAMixedBasisLocalView<GV, 2, 2> ;

    typedef TypeTree::CompositeNode<PQ2NodalBasisLeafNode<GV>,MAMixedHessianTree<GV, 2, 2> > Base;
    MAMixedBasisTree(PQ2NodalBasisLeafNode<GV> & uleafNode, const MAMixedHessianTree<GV, 2, 2> & hessianTree) :
