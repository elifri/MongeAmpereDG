...
\begin{frame}{Existenz von Lösungen}
	...
	\begin{itemize}
		...
		\pause
		\item keine generelle schwache Formulierung im klassischen Sinne\\
...
Vorteile der FDS:
\begin{itemize}
	\item Schema für Konvergenzbeweise
...
\item konvergiert langsamer als ein Newton-Verfahren, konvergiert für mehr Startwerte %\cite{Deblois1997}
...
\begin{frame}{Picard Iteration zur Lösung der MA-Gleichung}
	...
	\begin{itemize}
		...
		\pause
		\item unstetige Elemente, da $\cofHessH{u_h^n}$ unstetig ist (für finite Elemente, die nicht in $C^2()\Omega)$ enthalten sind)
	\end{itemize}
...
\pause
	\begin{itemize}
		...
		\item $a$ ist bilinear
		\pause
		\item $a$ ist beschränkt bzgl. der Energie-Norm $\eNorm \cdot$, falls $\sigma \geq \sigma^*$, d.h. $a(u,v) \lesssim \eNorm u \eNorm v $
		\pause
		\item $a$ ist koerziv bzgl. der Energie-Norm $\eNorm \cdot$, falls $A$ pos. def ist und $\sigma \geq \sigma^*$, d.h. es existiert $\alpha > 0$ mit $ a(v,v) \geq \alpha \qquad \forall v \in V_h {\eNorm v}^2$
		\pause
...
\begin{itemize}
	...
	\pause
	\item beschränke die Koerzitivitätskonstante von unten mit einer positiven Konstante $\varepsilon$
\end{itemize}
	\pause
