...
\subsection{Methods based on Variational Principles and Approximating Infinite-dimensional Spaces by Finite-dimensional Spaces}
 We concentrate ourselves in this work to methods of the second area to which beside DG methods methods such as finite element methods, least squares methods and boundary element methods belong. Current DG methods we discuss in the next chapter \ref{ch:DGMongeAmpere} in detail.
 
% The least squares methodology for a \MA equation formulates the problem as follows \cite[3.2.1]{FGN2013}:
%Find $(u,p) \in V_g \times Q^+_f$ such that
 %\[
%	j(u,p) \leq j(v,q) \qquad \forall (v,q) \in V_g \times Q^+_f
 %\]
%or
%\[
%	(u,p) = \argmin_{(v,q) \in V_g \times Q^+_f} j(v,q)
%\]
%with 
%\[
...
%\]
 %and 
%\[
%Q^+_f=\{q \in [L^2(\Omega)]^{2\times2} , q_{12} =q_{21},  q_{11} > 0,q_{22} > 0, det(q) = f\}.
%\] 
