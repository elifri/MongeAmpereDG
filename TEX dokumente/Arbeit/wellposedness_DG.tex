...
%	Let $u$ be the solution of the generalised Poisson Problem \ref{def: General Poisson Problem} and $\sigma \geq \sigma^*$ of Theorem \ref{thm: SIPG stability}. Let $u_h \in V_h$ satisfy \eqref{eq: DG system}, namely
%	\[
...
\begin{lemma}[Approximation properties of $\mathcal P_h^k$]  \label{la: approximation properties energy norm}
...
%\begin{proof}
%	We denote $u-v_h$ by w and start with the estimate from \eqref{eq: estimate avg term} 
%	\begin{align}
%		\sum_{e \in \edges} \singleNorm e \LTwonormE{\average{\nabla w}}^2 
%		\leq& C \sum_{T \in \triang} \left( \LTwonormT{\nabla w}^2 +h^2 \HOnenormT{ \nabla w}^2   \right) \nonumber\\
%		\leq& C \sum_{T \in \triang} \left( \HOnenormT{w}^2 +h^2 \HTwoSeminormT{w}^2   \right).
%	\end{align}
%	By the trace estimate of Lemma \ref{la: trace estimate} and the definition of the jump (cf. Definition \ref{def: edge operators}) we also have the estimate 
%	\begin{align}
%		\sum_{e \in \edges} \frac 1 {|e|} \LTwonormE{\jump w}
%		\leq& C \sum_{T \in \triang} \left( \frac 1 {h^2} \LTwonormT{w}^2 +\HOnenormT{w}^2   \right) %\nonumber\\
%%		\leq& C \sum_{T \in \triang} \left( \frac 1{h^2}\LTwonormT{w}^2 + \HOnenormT{w}^2   \right).
%	\end{align}
%	
