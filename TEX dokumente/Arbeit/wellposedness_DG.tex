...
\begin{proof}
	...
	& \leq
	\left( \sum\limits_{e \in \edges} \frac {\sigma}{|e|} \LTwonormE{\jump {\varphi}}^2 \right)^{\frac 1 2}
	\sup\limits_{e \in \edges} \LTwonormE{\average{A}} \left( \sum\limits_{e \in \edges} \frac {|e|} \sigma \LTwonormE{\average{\nabla v}}^2 \right)^{\frac 1 2} \label{eq: CS estimate}
	\end{align}
	As mentioned before we will from now on denote positive terms depending only on $A$ with the Constant $C$ whose value may vary between two inequalities. Since the third and the last term in \eqref{eq:inner product SIPG} are the symmetric to the second and fourth summands, we analogously have for their sum
	\begin{align}
	\sum\limits_{e \in \edges}\int_{e} \frac 1 {|e|}\jump {v \average{A \nabla \varphi} } \leq&
	\left( \sum\limits_{e \in \edges} \frac {\sigma}{|e|}\LTwonormE{\jump {v}}^2 \right)^{\frac 1 2}
	...
	\leq& 
	\left( 
	\LTwonorm{\nabla \varphi}^2+2\sum\limits_{e \in \edges} \frac {\sigma} {|e|}\LTwonormE{\jump {\varphi}}^2+ C\sum\limits_{e \in \edges} \frac {|e|} \sigma \LTwonormE{\average{\nabla \varphi}}^2
	\right)^{\frac 1 2} \nonumber \\
	&\times
	\left( 
	\LTwonorm{\nabla v}^2+2\sum\limits_{e \in \edges} \frac {\sigma}{|e|}\LTwonormE{\jump {v}}^2+ C\sum\limits_{e \in \edges} \frac {|e|} \sigma \LTwonormE{\average{\nabla v}}^2
	\right) ^{\frac 1 2} \nonumber \\
	\leq & C \eNorm \varphi \eNorm v 
	\end{align*}
\end{proof}
...
\begin{theorem}[Stability]\label{thm: SIPG stability}
	...
	\end{align*}
	for a positive Constant $C$ depending only on the mesh. 
...
\begin{proof}
	...
	Therefore we find
	\begin{align}
	a_h^{DG}(v_h,v_h) \geq \lambda \LTwonorm{\nabla v_h} - 2 \sum\limits_{e \in \edges}\int_{e} \jump {v \average{A \nabla v_h} } + \sum_{e\in \edgesi} \frac \sigma {|e|} \int_e \jump {v_h}^2 .\label{eq: coerc first estimate}
	\end{align}
	We derived in \eqref{eq: CS estimate} the estimate 
	\begin{align}
	\sum\limits_{e \in \edges}\int_{e} \jump {v_h \average{A \nabla v_h} }
