...
Therefore, we have to show the following statement
...
To estimate the remaining terms we need a standard trace and inverse estimate as can be found in \cite{BS2002}.
...
%	\leq&
%		C \left( \sum\limits_{T \in \triang}\frac 1 {h_T^2}\LTwonormT{ {\varphi}}^2 
%		+ \HOnenormT{ {\varphi}}^2 \right)^{\frac 1 2} \nonumber \\
%		&\cdot \left( \sum\limits_{T \in \triang}\frac 1 {h_T^2}\LTwonormT{\average{A \nabla v}}^2 
%		+ \HOnenormT{\average{A \nabla v}}^2 \right)^{\frac 1 2} \\
	\leq&
...
\begin{align}
\sum\limits_{e \in \edges}\int_{e} \frac 1 {|e|}\jump {\varphi \average{A \nabla v} } \leq&
	C \HOnenorm{\nabla \varphi}\HOnenorm{\average A}\HOnenorm{\average{\nabla v}}\label{eq: estimate second fourth term}
\end{align}
...
C \HOnenorm{\nabla v}\HOnenorm{\average A}\HOnenorm{\average{\nabla \varphi}}. \label{eq: estimate third fifth term}
\end{align}
