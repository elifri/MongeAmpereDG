...
\section{Implementation of a Finite Element Method}
...
To reduce the storage space one usually specify a reference cell $T_{ref}$ such that every triangle $T \in \triang$ is the image of $T_{ref}$ under an affine transformation $\Phi_T$. Choosing a polynomial basis $p^1_{ref},\dots,p^n_{ref}$ of $T_{ref}$ induces a basis on every $T$, namely 
\[
...
Most data on a triangle $T$ can be calculated knowing $\Phi_T$ and the reference cell $T_{ref}$. We explain how to do that exactly for the two-dimensional case.
...
\end{align*}
Thus, the function values in $T$ can be simply determined with barycentric coordinates. We refer from no on to the point $\beta_0 \point 0 0 +\beta_1 \point 0 1 + \beta_2 \point 1 0$ with $x_{ref}$.
...
Another crux of the implementation are of course the face terms. While assembling the matrices each face has to be naturally processed only once. One has to take that the normal derivatives are stored always with respect to 
