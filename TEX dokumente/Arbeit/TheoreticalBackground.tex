...
First, we can reduce all general Poisson problems with Dirichlet boundary data to \emph{problems with homogen boundary data}, i.e. $g \equiv 0$. Let us assume an $\tilde g \in H^1(\Omega)$ such that $g=\tilde g$ on $\partial \Omega$. Hence we can rewrite the problem as: Find  $u-g \in H^1_0(\Omega)$ such that  $a(\varphi,u)  = b(\varphi) \;\forall \varphi \in C^\infty(\Omega)$ holds.
