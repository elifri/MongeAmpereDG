...
\begin{align}
	H^s(\Omega;\triang) = \prod\limits_{T\in \triang}  H^s(T), \qquad W^{s,p}(\Omega;\triang) = \prod\limits_{T\in \triang} W^{s,p}(T).
\end{align}	
\end{definition}
...
\begin{align}
	J^\sigma(\varphi, v) = \sum\limits_{e \in \edges} \int_e \frac \sigma {|e|} \jump \varphi \jump v \textnormal{ and } 	J^\sigma_0(\varphi, v) = \sum\limits_{e \in \edgesb} \int_e \frac \sigma {|e|} \varphi g . \label{eq: penalty term}
\end{align}

Thus, we end up with the problem finding $v \in V_h$ such that
\begin{align}
	a_h^{DG}(\varphi, v) = l^{DG}_h (v) \qquad \forall \varphi \in V \label{eq: DG system}
\end{align}
where
\[ 	
	a_h^{DG} (\varphi, v) := a_S(\phi,v) + J^\sigma(\varphi,v) \text{ and } l_h^{DG}(\varphi) := l(\varphi) + J^\sigma_0(\varphi).
\]
