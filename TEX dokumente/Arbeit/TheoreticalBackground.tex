...
\begin{definition}[Triangulation, {\cite[Definition 5.1]{Braess2003}}]
	We call a finite partition of $\Omega$ into triangles $\{T_1, \dots, T_n\}$ a feasible \emph{triangulation} $\triang$, such that
	\begin{enumerate}[(i)]
		\item	$\bar \Omega = \bigcup_{T \in \triang} T_i$
		\item If $_i \cap T_j$ is a point for, it is a corner of both $T_i$ and $T_j$
		\item If $_i \cap T_j$ is more than one point for $i \neq j$, it is an edge of both $T_i$ and $T_j$
	\end{enumerate}
	$h$ denotes the maximal diameter $h:= \operatorname{max}\limits_{T\in \triang} diam(T)$ in the triangulation $\triang$. We call a triangulation \emph{regular} if every element is congruent.
\end{definition}
...
Figure \ref{fig: triangulation} shows a triangulation of the unitsquare. 
