At the beginning of this chapter we repeat the basis definitions for the analysis of partial differential equations as they are introduced for example in \cite{Evans1998}.
Let $\Omega \subset \R^d $ be an open area.
...
A function $u:\Omega \rightarrow \R$ fulfilling \eqref{eq:general PDE} is called a classical solution of the PDE. 
\todo{boundary conditions}

There are problems such that no such solution exists, often the demand on the high derivatives cannot be fulfilled. To admit less regular solutions one aims for a weaker solution notion. To do this we need to specify some spaces where to search for such weaker solutions.
...
\todo{Sollte ich  distributional derivates erklaeren?} The special case $p=2$ are referred to as Sobolev spaces $W^{s,2}(\Omega)=H^s(\Omega)$.

\todo{triangulation}
Let $\triang$ be a shape regular, quasi-uniform simplicial triangulation of $\Omega$, for simplicity reasons we assume that $\triang$ consists only of triangles.  For a triangle spanned by the points $v_0, v_1, v_2$ we also write 
\[
	T= \langle v_0, v_1, v_2 \rangle := \{\beta_0 v_0+ \beta_1 v_1 +\beta2 v_2; 0 \leq \beta_i \leq 1, i= 0,1,2\}
\]
\todo{diameter of a triangle}

We divide the edges $\edges$ into the subsets $\edgesi$ and $\edgesb$ containing all interior edges and boundary edges, respectively. On this basis we define the piecewise spaces
...
\begin{align*}
	\llbracket u_h \rrbracket &:= u_h^+ \cdot \mathbf n^+ + u_h^-\cdot \mathbf n^-  &&\text{ if } \partial T^+ \cap \partial T^- = e, \\
...
Due to the main theorem of variational analysis every solution to \eqref{eq: poisson eq} is equivalent to 
\begin{align}
...
\end{align}
The last statement can be reformulated to the variational form 
\begin{align}
...
\end{definition}
The generalised poisson problem is well-posed in the sense that it has a unique weak solutions for $A$ positive definite and $f\in L^2(\Omega)$. This is mainly due to the ellipticity of the PDE (and as a consequence thereof the ellipticity of $a$) allowing to apply the Lax-Milgram theory and a maximum principle. For a detailed analysis we refer the interested reader to \cite[Chapter~6]{Evans1998}.

On the left-hand side in the weak notation only first derivatives of $u$ appear, hence \eqref{eq: variational formulation} is also sensible for functions not twice differentiable. It turned out that the previously defined Sobolev spaces are well-suited candidates for the space $X$ where we find weak solutions. Therefore we search for the solution of the infinite-dimensional problem: Find $u\in H^1(\Omega)$ such that  $a(\varphi,u)  = b(\varphi) \;\forall \varphi \in C^\infty(\Omega)$ holds. % $\forall \varphi \in C^\infty(\Omega)$.
...
The limited space $W_h$ is called \emph{test space} and its elements are called \emph{test functions}. $V_h$ is referred as \emph{ansatz} or \emph{trial space}, the contained functions accordingly \emph{ansatz} or \emph{trial functions}. To assert the boundary condition \eqref{eq: poisson bc} we demand $u_h|_{\partial \Omega} = g$ for every $u_h \in V_h$.
...
\begin{lemma}[C\'ea Lemma{, \cite[4.2]{Braess2003}}]
	...
	\[
		a(u,u) \geq \alpha  \norm u ^2_m \text{ and } |a(u,v)| \leq C \norm u ^2_m \norm v ^2_m.
	\]
	Then for the solution $u_h$ of \eqref{eq: FE variational formulation}  holds
	\begin{align}
		\norm {u - u_h}_m \leq \frac C \alpha \inf\limits_{v_h \in V_h} \norm {u-v_h}_m.
	\end{align}
\end{lemma}
...
A recent idea is to choose the test and ansatz spaces to include discontinuous functions. This approach arised in the context of hyperbolic PDEs of which solutions eventually contain discontinuities. Although the Generalised Poissson Problem is not hyperbolic and its solutions are continuous we use this example to explain the issues arising when the finite element spaces is extended to discontinuities. The crux is the handling of derivatives along discontinuities.
...
\end{figure}
Typical formulas, i.e. quadrature points and weights, especially for handling volume integrals can be found in \cite{Strout1971}.
...
Another crux of the implementation is the handling of face terms. 
