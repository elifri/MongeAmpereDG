...
Since the influence of the gradient penalty term (cf. Section \ref{subsec: add penalty param}) on the solution is not clear, Figure \ref{fig: sigma variation} shows runs with different choices of $\sigma^G$.
\begin{figure}[H]
	\centering
	\includegraphics[scale =0.4]{plots/MA1_deg2_P.pdf}
...
\begin{table}[H]
	...
	columns/1/.style={column name={$\alpha$=0.1 }},
	columns/2/.style={column name={$\alpha=0.3$ }},
	columns/3/.style={column name={$\alpha=0.5$ }},
	columns/4/.style={column name={$\alpha=0.7$ }},
	every first column/.style={string type},
	%		every/.style={
	%		dec sep align,      % align on the decimal marker
	/pgf/number format/sci e, 
	...
	%	}
		...
		{$h$} 1 2 3 4
		{1/2} 0.0199431 0.022534 0.0192453 0.010306
		{1/4} 0.00451277 0.00441324 0.00403522 0.00641527
		{1/8} 0.00139369 0.000528091 0.000857927 0.00219968
		{1/16} 0.000667909 0.000127571 0.000334139 0.000576772
		{1/32} 0.000332824  {5.91847e-05} 0.264761 0.0569364
		{1/64} 0.000320034 0.104151 0.336712 0.0634047
	}
	\caption{$L^2$ errors for different choices of $\alpha$}
	\label{tab: sigma variation}
\end{table}

Further, to test the choice of the parameter $\alpha$ test case \ref{test smooth} was carried out for different choices of $\alpha$. The results can be found in Figure \ref{fig: alpha variation} and Table \ref{tab: alpha variation}. %, Figure \ref{fig: alpha close up} shows a Close up of the first iterations.
...
As to expect a larger choice of $\alpha$ leads to a faster decrease of the error. But as in the previous setting at same point all methods become instable diverge. Even the method with $\alpha=0.3$ begins in the last refinement to decrease, although the error is not to identify in the plot shown in Figure \ref{fig: alpha variation}.

Surprising is the fact the solution converge against different solutions: After 15 iterations, i.e. all data points before $h=1/4$ every method stagnates at a different $L^2$ error. This repeats itself after 30 iterations, i.e. before $h=1/8$. Although for $\alpha=0.7$ we had the smallest error on the coarsest grid, after 30 iterations this method yields the largest error.
Since all methods use the same discretisation we can exclude the error after 15 Picard iterations is due to the discretisation error. It may be possible those method found different fixed points. Since the error decreases after a refinement these fixed points have to be connected to the exact solution and $h$.
...
Additionally one can in the FEniCS variant the cofactor matrix of the Hessian which is evaluated piecewise replace by the discrete Hessian as defined in \ref{def: discrete Hessian} for a symmetric ansatz space $\Sigma_h$. The code can be found in the Appendix \ref{app: our code}.