...
The arising nonlinear system given by \eqref{eq: brenner method} is solved by the scientific tool kit PETSc (cf. \cite{petsc}) configured with FEniCS default solver choice: Which is a Newton-based nonlinear solver that uses a line search and solves arising linear systems by a $LU$ decomposition.
...
For the second test case Newton's method did not converge, not even on the coarsest grid. Also in the third test case our Newton solver was not able to obtain results. Table \ref{tab: l2 errors test 3 Brenner} presents the results of the runs for $h=1/2, \dots, 1/16$, for finer grids the solver did not converge. Attempts to solve this example with FEniCS' built in damped Newton method with different relaxation parameters did neither succeed on any grid finer than $h=1/16$.
\begin{table}[h]
\centering
	\begin{subtable}[b]{0.45\textwidth}
		\centering
...
The fourth example does not have a classical solution. As in the previous cases the method does not converge. The corresponding results are shown in Table \ref{tab: l2 errors test 4 Brenner}.
...
\begin{table}[H]
	\begin{subtable}[b]{0.45\textwidth}
		...
		\pgfplotstabletypeset[columns={iterations, l2error, h1error,N},
					...
					sci,sci zerofill,sci sep align,
			        /pgf/number format/sci e, 
			        /pgf/number format/sci precision=6 ,    % print 4 digits
			        /pgf/number format/precision=6     % print 4 digits
				    },
		]\MAFourBrennerTwo
    	\caption{Error for $k=2$}
   \end{subtable}
...
Although the convexification algorithm produces a convex result it does not guarantee that it leaves convex input functions unchanged.
...
%	columns/0/.style={column name=$h$},
	columns/0/.style={column name={$\sigma^G$=0 }, sci,sci zerofill,sci sep align, sci e},
	columns/10/.style={column name={$\sigma^G$=10 }, sci,sci zerofill,sci sep align, sci e},
	columns/30/.style={column name={$\sigma^G=$30 }, sci,sci zerofill,sci sep align, sci e},
	columns/50/.style={column name={$\sigma^G=50$ }, sci,sci zerofill,sci sep align, sci e},
	columns/70/.style={column name={$\sigma^G=70$ }, sci,sci zerofill,sci sep align, sci e},
	columns/100/.style={column name={$\sigma^G=100$ }, sci,sci zerofill,sci sep align, sci e},
	columns/200/.style={column name={$\sigma^G=200$ }, sci,sci zerofill,sci sep align, sci e},
