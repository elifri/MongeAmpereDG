In this chapter we develop a \emph{Picard iteration} aiming to solve the \MA equation. To the author's knowledge this is the first attempt to apply the \emph{Picard linearisation} on the \MA equation. 
...
\] 
This kind of linearisation is also called \emph{Picard linearisation} method and is very convenient to linearise the diffusion term in the Navier-Stokes equations.

In this spirit we aim at decoupling the derivatives in the strongly nonlinear \MA equation such that the decoupled PDEs are linear. 
...
\end{align}
Note that this derivation of decoupling of $v$ and $w$ is chosen because it is very intuitive. Alternatively, \eqref{eq:short formula} can be derived directly from the identity $\mycof {D^2 u }:D^2 u  = 2 \mydet{D^2 u}$ stated in Lemma \ref{la: rel det cofactor}.
...
Furthermore, as it is easy to see in \eqref{eq:decoupled PDE start} the left-hand side is symmetric in $v$ and $w$. Thus, it is natural to examine a sequence of PDEs where alternately either $v$ or $w$ is fixed. 
...
Taking a closer look at the derivation of \eqref{eq:decoupled PDE} we see only the smoothness of $w$ has been used. Hence, for any smooth solution $u$ the \MA equation can also be written as
\begin{align}
- \nabla \cdot \left( \mycof{ D^2 u} \nabla u \right)  = -2f.  \label{eq: analytical fixed point iteration}
\end{align}
This also implies that every solution of a \MA problem is a fixed point of the Picard iteration. 
We can formulate \eqref{eq: analytical fixed point iteration} weakly: By integration by parts and forcing the boundary conditions with a penalty term we obtain for a test space $V=H^2(\triang; \Omega) \cap H^1$
\begin{align*}
	\myIntX{\Omega} {\mycof{ D^2 u} \nabla u \cdot \nabla v} 
	-\sum\limits_{e \in \edgesb}\myIntS e { \jump {v \average { \cof(D_h^2 u) \nabla u} }}& \\
	+ \sum\limits_{e \in \edgesb} \myIntS e { \frac \sigma {|e|} v (u-g)}& 
	= \myIntX \Omega {-2f v} \qquad \forall v \in V.  
\end{align*}

It remains to test if the Picard iteration converges to its fixed point and hence yields an approximation of the solution of the \MA equation. 
...
\section{Challenges for the Picard Iteration}
...
%The solution of \eqref{eq: start sqrt_f} serves as an initial guess, 
...
Nevertheless, starting over the test with the initial guess from \eqref{eq: start sqrt_f} instead of the exact solution the method is unstable and diverges. But the results yet are very remarkable. As shown in Figure \ref{fig: oscillation} the error is oscillating while diverging  to infinity.
...
\subsection{Convexification} \label{subsec: convexification}
...
we obtain for example for the B\'ezier coefficients $c_{ijk}$.
 ...
 For the following theorems  we always refer to the same setting: We have a triangle $T=\langle v_1, v_2,v_3 \rangle$ and a polynomial $p$ of degree $d$ defined on this triangle $T$. Further the B\'ezier coefficients of $p$ are denoted by $c_{ijk}$.
...
\]
Geometrically in the case of $d=2$ this ensures the control point $Q_{101}$ (cf. \eqref{eq: control points}) lies above the plane where the triangle $\langle Q_{200}, Q_{110}, Q_{101} \rangle$ is embedded. %Note that the first two coordinates are the coordinates of the domain points. Thus, we can identify the 
