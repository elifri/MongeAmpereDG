...
In \cite{Awanou2014} Awanou analysed an iteration process similar to the just presented Newton iteration:
\begin{align}
...
The following lemma shows $\epsilonTwo{\cdot}$ is not only a measure for the cofactor of the Hessian, but also for the Hessian.
...
\end{lemma}
\begin{proof}
	Let $\triangM$ for $n \in \N$ be triangulations of $\Omega$. By Lemma \ref{la: approximation properties} there exists a sequence $p_n$ such that $p_n \in \mathcal {P}^2_{1/n}(\triangM)$ so that with respect to $\norm{\cdot}_{H^2(T)}$ it holds
	\[
		p_n \xrightarrow{n \rightarrow \infty} v \qquad \text{ on every } T \in \triang.
	\]	
	By the definition of the cofactor matrix (cf. Definition \ref{def: cof matrix}) we find
	\[
		\cofHess{p_n} \xrightarrow{n \rightarrow \infty} \cofHess{v} \qquad \text{ on every } T \in \triang
	\]	
	with respect to $\norm{\cdot}_{L^2(T)}$. Note that the entries of $\cofHess{p_n}$ are piecewise constant.
	
	Let $\lambda_w$ denote the function that maps $x$ to the greatest eigenvalue of $\cofHessH{w}$ evaluated at $x$. 
	Due to the choice of $p_n$, $\lambda_{p_n}$ is constant on every $\tilde T \in \triangM$ for every $n \in \N$. We denote the set of normalised, piecewise constant eigenvectors by
	\[
		M_n = \{v \in L^2(\Omega); \LTwonorm{v}=1, \cofHessH{p_n} v = \lambda_{p_n} v\}.
	\]	
	Hence, we find for $n \in \N$
	\begin{align*}
	\LTwonorm{ \cofHessH {p_n} } &=  \sup_{w \in L^2(\Omega), \LTwonorm{w}=1} \myIntX \Omega {w^t \cofHessH{p_n}^t \cofHessH{p_n} w} \\
	&= \sup_{w \in M_n} \sum_{\tilde T \in \triangM} \myIntX {\tilde T} {w^t \lambda_{p_n}^2 w} 
		= \sup_{w \in M_n}\sum_{\tilde T \in \tilde \triangM} \left. \lambda_{p_n}^2 \right\vert_{\tilde T} \norm{w}_{L^2(\tilde T)},
	\end{align*}
	Since the eigenvalues of $\cofHessH{p_n}$ and $\hess{p_n}$ coincide we obtain analogously 
	\begin{align}
	\LTwonorm{ \hess {p_n} } = \sup_{y \in N_n} \sum_{\tilde T \in \triangM} \left. \lambda_{p_n}^2\right\vert_{\tilde T} \norm{y}_{L^2(\tilde T)},\label{eq: sup cof hess}
	\end{align}
	where $N_n$ is the set of eigenvectors to $\hess v$, namely
	\begin{align}
			N_n = \{v \in L^2(\Omega); \LTwonorm{v}=1, \hess{p_n} \; v = \lambda_{p_n} v\}. \label{eq: sup hess}
	\end{align}
	Note that we have to show that these two suprema coincide.
	On one hand we have for every function $w \in M_n \cup N_n$ that $\sum_{\tilde T \in \triangM} \norm w _{L^2(\tilde T)} = 1$. On the other hand for every convex combination $\sum_{\tilde T \in \triangM} \alpha_{\tilde T} =1$  there exist a $w \in M_n \cup N_n$ such that $\alpha_{\tilde T} = \norm w _{L^2(\tilde T)}$. Thus, the two suprema in \eqref{eq: sup cof hess} and \eqref{eq: sup hess} have to be equal and we obtain 
	\begin{align*}
	 	\LTwonorm{\cofHessH{p_n}} = \LTwonorm{\hess{p_n}} \qquad \forall n \in \N. 
	 \end{align*}
	Taking again the limit $n \rightarrow \infty$ yields
	 \begin{align}
	 	\LTwonorm{\cofHessH{v}} = \LTwonorm{\hess{v}} \qquad \forall v \in V.
	 	\label{eq: equality norm}
	 \end{align}

	Analogously we obtain on edges
	\begin{align}
		 	\LTwonormE{\cofHessH{v}} = \LTwonormE{\hess{v}} \qquad \forall v \in V.
		 	\label{eq: equality norm edge}
	\end{align}
...
\begin{lemma}[Stability] \label{la: stability L}
	...
	\end{align}
	$C$ denotes a positive constant independent of $h$ and $r_h$. 
\end{lemma}
\begin{proof}
...
Note that this means $F_i$ defined in \eqref{eq: definition F} has a unique solution in $V_h$ as $R$ is constant in $w$.
...
Now we are able to establish an estimate of the consistency error of $F_i u$.
\begin{theorem}[Consistency Error of $F_i u$] \label{la: consistency error F}
...
\begin{proof}
	...
	\end{align}
	Using again the standard combination of the trace estimate from Lemma \ref{la: trace estimate} and the inverse estimate from Lemma \ref{la: inverse estimate}\footnote{for more detailed steps of the combination of trace and inverse estimate see the proof of Lemma \ref{la: equivalence energy norm}} we find
	\begin{align}
	\sum_{e \in \edgesb} \frac 1 {|e|} \LTwonormE{ \cofHessH{(u-u_h^i)}}^2 
	\leq C \sum_{T \in \triang} \LTwonormT{\cofHessH{(u-u_h^i)}}^2.
	\end{align}
...
\begin{lemma}[Contraction property of $\mathcal{M}$] \label{la: contraction property M}
	For any $w_1, w_2 \in V$ it holds for a positive constant $C$ independent of $h$
\begin{align*}
...
\begin{lemma}\label{la: difference u uch}
	...
	\]
	where $C>1$ denotes a constant independent of $h$, but dependent on $u$.
\end{lemma}
\begin{proof}
...
\begin{lemma}[Mapping Property of $\M$] \label{la: mapping property of M}
	...
	\]
	where $C$ is a positive constant independent of $h$, but dependent on $u$.
\end{lemma}
\begin{proof}
...
\begin{theorem}\label{main result}
	...
	Furthermore if there exists $C^*$ independent of $h$ such that
	\begin{align}
