...
Brenner et al. emphasis that it is important that the linearisation of a \MA method is a consistent and stable discretatisation of the linearisation of the \MA equation, namely $\nabla \cdot \left( \mycof {D^2 u } \nabla (v) \right)$ \cite[Remark 2.2]{BGN+2011}.
...
For the rest of the analysis, we consider a convex and polyhedral domain $\Omega\subset \R^2$ and $f,g$ such that the \MA problem then has a unique convex solution $u \in H^s(\Omega)$ with $s\geq 2$. 
...
\end{lemma}
\begin{proof}
	Let $w_1, w_2, v$ be in $V$. For any $\varphi_h \in V_h$ we have by the definition of $L$ in \eqref{eq: linearisation our} and linearity
	\begin{align}
...
Now we want to transfer the argument structure given by Brenner et al. So, analogously to their definition in \cite[(3.1)]{BGN+2011} we define the operator $\mathcal M: V \rightarrow V_h$ by
\begin{align}
...
We want to derive a contraction property as well as a mapping property to apply Banach's fixed point theorem. If we are able to show that $\M_h$ restricted to an arbitrary small set is a self-mapping we can locate the fixed point with arbitrary precision. As Brenner et al. \cite[(3.3)]{BGN+2011} we choose this set to be a small ball centered around $u_{c,h}\in V_h$, where
\[
...
\end{align*}
Applying Theorem \ref{thm: SIPG stability} and Lemma \ref{la: L dependence parameter} yields the following contraction result which is motivated by \cite[Lemma 3.4]{BGN+2011}.
...
\begin{lemma}\label{la: difference u uch}
	...
	\]
	where $C>1$ denotes a constant independent on $h$.
\end{lemma}
\begin{proof}
	For any $v_h \in V_h$ holds by applying the definition of $u_{c,h}$ and both statements given in Lemma \ref{la: stability L}
	\begin{align}
		\eNorm{u-u_{c,h}} \leq& \eNorm{u- v_h} + \eNorm{ v_h- L_{u,h}^{-1}L_u u} \nonumber \\
			\leq& \eNorm{u- v_h} + \eNorm{L_{u,h}^{-1}L_{u,h} v_h- L_{u,h}^{-1}L_u u} \nonumber \\
...
Let us now derive an estimate for the image of a small ball centered at $u_{c,h}$ inspired by the estimate \cite[Lemma 3.6]{BGN+2011}.
...
This result is inspired by Lemma 3.6 in \cite{BGN+2011}. Once again, we observe the differences of the two methods: While the statement in this Lemma relies on the previous consistency estimates, the estimate derived by Brenner et alt. depends on the contraction property of the nonlinear part $R^B$ from \eqref{eq: add brenner method}. 
...
\begin{theorem}\label{main result}
	...
	\end{align}
	there exist a $h^*> 0$ such that $u_h^i$ fulfils the requirements to apply \eqref{eq: error estimate} for all $h > h^*$.
...
\begin{proof}
	...
	\end{align}
	Thereby it follows from \eqref{eq: tau1} that $\M$ fulfils the contraction property stated in Lemma \ref{la: contraction property M} with the constant $\delta<1$.
	...
	By \eqref{eq: tau2} we have 
	 ...
	 \begin{align}
	 	\eNorm{u-u_h} \leq \eNorm{u-u_{c,h}} + \eNorm{u_{c,h} - u_h} \leq \inf_{v_h \in V_h} \eNorm{u-v_h} +\rho_0 
	\end{align}
	...
	\end{align*}
	Since analogously $\tau_1, \tau_2 \xrightarrow[]{h\rightarrow 0} 0$ holds there exist $h^*$ such that their maximum, i.e. $\delta$ is smaller than 1.
\end{proof}
...
\end{align}
where $l = \min\{k+1,t\}$. Thus, we have even for a reliable approximation of $u$ by Theorem \ref{main result} no satisfying result on the Picard Iteration. Furthermore, if we have a guess that fulfils \eqref{eq: estimate error} this theorem does not ensure $u_h$ is a better approximation to $u$ than $u^i_h$. 

The analogue result for the method of Brenner et al. \cite[Theorem 3.1]{BGN+2011} yields optimal convergence results, but they depend on the property that $l$ defined as in \eqref{eq: approx hess} is strictly greater than $3$ to control the mapping property of $\mathcal M$. However, we depend on the error in the second derivatives, i.e. $\epsilonTwo{u-u_h^i}$ to control this mapping property. The problem is that derived mapping property in Lemma \ref{la: mapping property of M} is not sufficient to imitate the results of Brenner et alt. as it does not decrease linearly with a power of $h$.

Unfortunately, also later numerical results suggest that this formulation of a Picard Iteration do not converge to the \MA solution.	
