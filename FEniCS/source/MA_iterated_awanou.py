...
def MA_iteration_Awanou(mesh, V, u0, f, max_it,w):
  ...
  a = inner(nabla_grad(v), coeff_u0*nabla_grad(u))*dx \
    - inner(jump(v,n),avg(coeff_u0*nabla_grad(u)))*dS\
    - inner(jump(u,n),avg(coeff_u0*nabla_grad(v)))*dS\
    + Constant(sigma)('+')/h('+')* jump(u)*jump(v)*dS \
    + Constant(sigma/2)('+')/h('+')*jump(nabla_grad(u),n)*jump(nabla_grad(v),n)*dS \
    - v*inner(n,coeff_u0*nabla_grad(u))*ds \
    - u*inner(n,coeff_u0*nabla_grad(v))*ds \
...
#    
  ...
  L = inner(-f-div(nabla_grad(w))+det(grad(grad(w))),v)*dx - u0*dot(n,coeff_u0*nabla_grad(v))*ds +Constant(sigma)/h *u0*v*ds
...
if __name__ == "__main__":
  ...
  w.assign(u_e)
  #w.assign(u_e+-0.01*interpolate(error, V))
