...
#-----------choose PDE------------
def MA_problem(name, Nh, degree, mesh, domain):
  
  #define rhs
  class rhs(Expression):    
    def __init__(self, mesh, domain, **kwargs):
        self._mesh = mesh
        self._domain = domain
    def eval(self, v, x):
      if name == 'MA2':
  ...
  if name=='MA1':
    ...
    f = rhs(mesh=mesh, domain=domain, element=element)
  elif name == 'MA3':
    f = rhs(element = FiniteElement("Quadrature", triangle, degree))
  elif name == 'const rhs':
    f = Constant(1.0)
  ...
  #-----------choose PDE------------
def MA_problemSimple(name, Nh, degree):
  ...
  if name=='MA1':
    ...
    u0  = exact_sol(element = FiniteElement("Quadrature", triangle, degree))
  elif name == 'MA4':
...
def MA_exact_derivativeEx(name):
  ...
  return Expression(((d00, d01, d10, d11)))
