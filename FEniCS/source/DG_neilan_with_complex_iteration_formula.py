...
def cofac_mult(A,b):
  return (as_matrix([[A[3],-(A[1]+A[2])/2.0],[-(A[1]+A[2])/2.0,A[0]]])*b)


def cofac(A):
  return [A[3],-(A[1]+A[2])/2.0,-(A[1]+A[2])/2.0,A[0]]
...
#  cofTimesDu = matrix_mult(cofac(w), nabla_grad(u))
 # cofTimesDv = matrix_mult(cofac(w), nabla_grad(v))
  cofTimesDu = cofac_mult(w, nabla_grad(u))
  cofTimesDv = cofac_mult(w, nabla_grad(v))

  #plot(project(determinant(cofac(w)),bigV), title = 'determinant entry of cof hessian')

  #--------------define function------------------------
#  F = 0 
  
  F = inner(cofTimesDv , nabla_grad(u))*dx \
      - v('+')*  dot(avg(cofTimesDu),n('+'))*dS \
      - v('-')*  dot(avg(cofTimesDu),n('-'))*dS \
      - v*inner(n,cofTimesDu)*ds \
      - u*inner(n,cofTimesDv)*ds \
      - u('+')*  dot(avg(cofTimesDv),n('+'))*dS \
      - u('-')*  dot(avg(cofTimesDv),n('-'))*dS \
      + Constant(sigmaB)/h*v*u*ds \
      + Constant(sigmaC)('+')/h('+')* jump(u)*jump(v)*dS \
#      + Constant(sigmaG)('+')*h('+')* jump(nabla_grad(u),n)*jump(nabla_grad(v),n)*dS \

  F = F - inner(Constant(-2.0)*f,v)*dx \
      + u0*dot(n,cofTimesDv)*ds \
      - Constant(sigmaB)/h *u0*v*ds
...
#  assign(u_.sub(1), [u0_derivXX_e, u0_derivXY_e, u0_derivXY_e, u0_derivYY_e])
 ...
 # plot(project(abs(u_.sub(1)[0]-u0_derivXX_e),bigV), title = 'first entry of hessian error')
  ...
  plot(project(determinant(cofac(u_.sub(1))),bigV), title = 'determinant entry of cof hessian')
