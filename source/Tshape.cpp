...
void Tshape::initialize_mass() {
	...
	assert (shapedim == shapeSigmadim && " i don't have enough quadrature data for different polynomial degrees");

	if (shapedim == shapeSigmadim)
		mass_hessian.set_A(A);
	else
	{
		for (unsigned int i = 0; i < shapeSigmadim; i++) {
			for (unsigned int j = 0; j < shapeSigmadim; j++) {
...
void Tshape::assemble_hessian(const Ediffusionmatrix_type & A, const unsigned int &istate, Equadratureshape_hessian_type &hess) const{
	...
	for (int iq = 0; iq < Equadraturedim; ++ iq){
		...
		for (int ishape = 0; ishape < shapeSigmadim; ++ishape) {
			hess(iq) += A(ishape) * get_equad_data().Equads(ishape, iq);
		}
//		cout << "hess at iq " << iq << " is " << hess(iq) << endl;
	}
}
