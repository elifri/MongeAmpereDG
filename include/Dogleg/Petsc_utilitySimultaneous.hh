...
public:
	...
	///provides data for information exchange to form functions
	PetscSolverData data;
...
//  ierr = VecDuplicate(x,&U);CHKERRQ(ierr);
  ...
  */
  ierr = SNESSetFunction(snes,r,FormFunction, &data);CHKERRQ(ierr);
...
//  ierr = MatSeqAIJSetPreallocation(J,0,guess_nonzeros_J.data());CHKERRQ(ierr);
  ierr = MatSeqAIJSetPreallocation(J,PETSC_DEFAULT,NULL);CHKERRQ(ierr);
  std::cout << "guess " << guess_nonzeros_J.maxCoeff() << std::endl;
  ...
  */
  ...
  ierr = SNESSetJacobian(snes,J,J,FormJacobian,&data);CHKERRQ(ierr);
...
PetscErrorCode PETSC_SNES_Wrapper<OperatorType>::FormFunction(SNES snes,Vec x,Vec f,void *ctx)
{
  ...
  PetscSolverData* data_ptr = static_cast<PetscSolverData*>(ctx);

	Solver_config::VectorType f_eigen(n);
	op.evaluate(v,f_eigen, data_ptr->J, v, false);
...
PetscErrorCode PETSC_SNES_Wrapper<OperatorType>::FormJacobian(SNES snes,Vec x,Mat jac,Mat B,void *dummy)
{
	...
	op.solver_ptr->update_solution(v);
  ...
  /*
     Compute Jacobian entries and insert into matrix.
      - Note that in this case we set all elements for a particular
        row at once.
  */
	PetscSolverData* data_ptr = static_cast<PetscSolverData*>(dummy);

	assert(data_ptr->J.nonZeros() < 2147483647);
  std::cout << " J in Jacobian nonzeros " << data_ptr->J.nonZeros() << std::endl;

	ierr = from_eigen_to_petscmat(data_ptr->J, jac);
