...
#ifndef DUNE_SSplineGlobal_HH
#define DUNE_SSplineGlobal_HH
...
/**@ingroup LocalBasisImplementation
 \brief PowellSabin 12 Split (Macro element) shape functions of piecewise quadratic order on the reference triangle.

  The macro element is described by control nodes
...
template<class Geo, class D, class R>
...
/*  !
  ...
  struct Traits{
    ...
    typedef FieldMatrix<RangeField, dimDomainGlobal, dimDomainGlobal> Hessian;
    typedef Hessian HessianType;
...
//  PS12SSplineGlobalBasis () : geo_(&(new Geometry)), A_(&(new SparseMatrixType))  {}
  ...
  PS12SSplineGlobalBasis (const Geometry &geo) : geo_(geo)
  {
...
*/
      ...
      std::vector<typename Traits::Range>& out) const
  {
    ...
    //use Corollary 2.8 from Paper Simplex Splines for the Powell-Sabin 12-Split Elaine Cohen, Tom Lyche and Richard Riesenfeld
...
//    std::cout << " barycPos " << barycPos << std::endl;
    ...
    //determine subtriangle no and indices of splines with support in \triang_k
    int k;
    std::array<int, 3> gk1; //indices of linear splines with support in \triang_k
    std::array<int, 6> gk2; //indices of quadratic splines with support in \triang_k
...
//    std::cout << " R1 " << Rk1 << std::endl;
    //calculate recurrence (sub)matrix for quadratic splines
    FieldMatrix<R, 3 , gk2size> Rk2;
    for (int i = 0; i < 3; i++)
      for (int j = 0; j < gk2size; j++)
        Rk2[i][j] = R2(barycPos, gk1[i], gk2[j]);

    const auto basisValuesWithSupport = Rk1.rightmultiplyany(Rk2);
      ...
      std::vector<typename Traits::Jacobian>& out) const// return value
  {
	...
	//start algorithm 4.4 from paper Simplex splines for the powell-sabin 12-split
    out.resize(N);
    std::fill(out.begin(), out.end(), 0);
...
/*    //caluclate recurrence (sub)matrix for quadratic splines
    ...
    for (unsigned int dim = 0 ; dim < Traits::dimDomainLocal; dim++)
    {
      auto basisValuesWithSupport = Rk1.rightmultiplyany(Uk2[dim]);
  ...
  //! \brief Evaluate higher derivatives of all shape functions
                        ...
                        std::vector<typename Traits::Range>& out) const //return value
   {
     ...
     int directionU2 = dOrder == 1? directions[0]: directions[1];

     FieldMatrix<R, 3 , gk2size> Uk2;
     for (int i = 0; i < 3; i++)
       for (int j = 0; j < gk2size; j++)
         Uk2[i][j] = U2(directionAxis_[directionU2], gk1[i], gk2[j]);
     ...
     else if (dOrder==1)
     {
       ...
       auto basisValuesWithSupport = Rk1.rightmultiplyany(Uk2);
       basisValuesWithSupport*=2;

       for (int i = 0; i < gk2size; i++)
         out[gk2[i]][0] = basisValuesWithSupport[0][i];
     }
     else if (dOrder==2)
     {
       FieldMatrix<R, 1 , 3> Uk1;
       for (int j = 0; j < 3; j++)  Uk1[0][j] = U1(directionAxis_[directions[0]], k, gk1[j]);

       auto basisValuesWithSupport = Uk1.rightmultiplyany(Uk2);
       basisValuesWithSupport*=2;
   ...
   //! \brief Evaluate higher derivatives of all shape functions
    inline void evaluateAll(const typename Traits::DomainLocal& x,  //position
                         std::vector<typename Traits::Range>& outD0,
						 std::vector<typename Traits::Jacobian>& outD1,
						 std::vector<typename Traits::Hessian>& outD2) const //return value
    {
        outD0.resize(N);
        outD1.resize(N);
        outD2.resize(N);
        std::fill(outD0.begin(), outD0.end(), 0);
        std::fill(outD1.begin(), outD1.end(), 0);
        std::fill(outD2.begin(), outD2.end(), 0);

        //start algorithm 4.2 from paper Simplex splines for the powell-sabin 12-split
         //(1)get barycentric triangle coordinates
         BarycCoordType barycPos;
         calcBarycCoords(x, barycPos);

         //(1)determine subtriangle no and indeces of splines with support in \triang_k
         int k;
         std::array<int, 3> gk1; //indeces of linear splines with support in \triang_k
         std::array<int, 6> gk2; //indeces of quadratic splines with support in \triang_k
         ...
         //calc s^k_1 for r=0,1
         for (int j = 0; j < 3; j++) s[gk1[j]][1] = s[gk1[j]][0]*R1(barycPos, k, gk1[j]);
     ...
     //    std::cout << " R1 " << Rk1 << std::endl;
         ...
         for (int j = 0; j < gk2size; j++)
        	 for (int k =0; k < 3; k++)
        	 {
        	   outD0[gk2[j]] += s[gk1[k]][1]*Rk2[k][j];
        	 }

         //(4)
         //calculate derivative (sub)matrix for linear splines
         FieldMatrix<R, 1 , 3> Uk1Dx0, Uk1Dx1;

         for (int j = 0; j < 3; j++)
         {
        	 Uk1Dx0[0][j] = U1(directionAxis_[0], k, gk1[j]);
        	 Uk1Dx1[0][j] = U1(directionAxis_[1], k, gk1[j]);
         }
     //    std::cout << " U1 " << Uk1[0] << std::endl << " and " << Uk1[1] << std::endl;

         //caluclate derivative (sub)matrix for quadratic splines
         FieldMatrix<R, 3 , gk2size> Uk2Dx0, Uk2Dx1;
         for (int i = 0; i < 3; i++)
           for (int j = 0; j < gk2size; j++)
           {
        	   Uk2Dx0[i][j] = U2(directionAxis_[0], gk1[i], gk2[j]);
             Uk2Dx1[i][j] = U2(directionAxis_[1], gk1[i], gk2[j]);
           }

         //calc s^k_2 for r=1  Dx
         for (int j = 0; j < gk2size; j++)
        	 for (int k =0; k < 3; k++)
        	 {
        	   outD1[gk2[j]][0][0] += 2*s[gk1[k]][1]*Uk2Dx0[k][j];
             outD1[gk2[j]][0][1] += 2*s[gk1[k]][1]*Uk2Dx1[k][j];
        	 }

         //calc s^k_1 for r=2
         for (int j = 0; j < 3; j++) s[gk1[j]][1] = s[gk1[j]][0]*Uk1Dx0[0][j];

         for (int j = 0; j < gk2size; j++)
        	 for (int k =0; k < 3; k++)
        	 {
        		 outD2[gk2[j]][0][0] += 2*s[gk1[k]][1]*Uk2Dx0[k][j];
        		 outD2[gk2[j]][0][1] += 2*s[gk1[k]][1]*Uk2Dx1[k][j];
        	 }

         //calc s^k_1 for r=2
         for (int j = 0; j < 3; j++) s[gk1[j]][1] = s[gk1[j]][0]*Uk1Dx1[0][j];

         for (int j = 0; j < gk2size; j++)
        	 for (int k =0; k < 3; k++)
        	 {
        		 outD2[gk2[j]][1][0] += 2*s[gk1[k]][1]*Uk2Dx0[k][j];
        		 outD2[gk2[j]][1][1] += 2*s[gk1[k]][1]*Uk2Dx1[k][j];
        	 }
    }
