...
public:
  ...
  void initializeIndices()
  {
    ...
    innerOffset_ = gridView_.size(dim-1)*dofPerEdge_;
  }
...
public:
  ...
  MultiIndex index(size_type i) const
  {
    ...
    if (i % 4 == 1) //degree of freedom associated to a vertex
    {
      ...
      if (i % 4 != 3) //degree of freedom associated to a edge
      {
    ...
    auto cell_id = gridIndexSet.subIndex(element,
        fe.localCoefficients().localKey(i).subEntity(),
        fe.localCoefficients().localKey(i).codim());
    return {{nodeFactory_->innerOffset_+cell_id*NodeFactory::dofPerCell_+ fe.localCoefficients().localKey(i).index()}};
  }
