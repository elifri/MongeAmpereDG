...
private:
  ...
  using PQdegHessFactory = PQkNodeFactory<GV,degHess,PQMultiIndex,ST>;
...
protected:
  ...
  PQdegHessFactory uDHFactory_;
};

template<typename GV, int degHess, class ST, typename TP>
  class MAMixedHessianTree: public PowerBasisNode<ST, TP, PQkNode<GV, degHess, ST, decltype(TypeTree::push_back(TP(), 0))>, GV::dimension*GV::dimension>
  {
    using ComponentTreePath = decltype(TypeTree::push_back(TP(), 1));
    using PkNode = PQkNode<GV,degHess, ST, ComponentTreePath >;
    using Base = PowerBasisNode<ST, TP ,PkNode, GV::dimension*GV::dimension>;
  ...
  public:
      ...
      Base(tp)
    {
      for(int i=0; i<GV::dimension*GV::dimension; ++i)
        this->setChild(i, std::make_shared<PkNode>(TypeTree::push_back(tp, i)));
    }
};
...
public:
  ...
  using uDHNodeIndexSet = typename NodeFactory::PQdegHessFactory::template IndexSet<uDHTreePath>;
