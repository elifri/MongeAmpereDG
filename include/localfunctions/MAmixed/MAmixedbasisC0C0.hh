...
public:
  ...
  MultiIndex index(size_type localIndex) const {
    ...
    if (mi[0] == 0) {
      ...
      size_type uDH_comp = (localIndex - u_size) / nDH;
      size_type uDH_localIndex = (localIndex - u_size) % nDH;
      mi[1] = nDH * uDHLocalIndexSet_.index(uDH_comp)[0] + uDH_localIndex;
    }
    return mi;
  }
...
private:
  ...
  typename PQKNodalBasis<GV, degHess>::IndexSet::LocalIndexSet uDHLocalIndexSet_;
};
...
private:
  ...
  typename PQKNodalBasis<GV, degHess>::IndexSet uDHIndexSet_;
};
...
class MAMixedBasis: public GridViewFunctionSpaceBasis<GV,
    MAMixedBasisLocalView<GV, deg, degHess>, MAMixedIndexSet<GV, deg, degHess>,
...
public:
  ...
  typedef PQKNodalBasis<GV, degHess> BasisuDH;
...
private:
  ...
  PQKNodalBasis<GV, degHess> uDHnodalbasis_;
};
...
public:
  ...
  MAMixedBasisLocalView(const GlobalBasis* globalBasis) :
      globalBasis_(globalBasis), ulocalView_(
          globalBasis->unodalbasis_.localView()), uDHlocalView_(
          globalBasis->uDHnodalbasis_.localView()), uDHTree_(
          uDHlocalView_.tree()), tree_(ulocalView_.tree(), uDHTree_) {
  }
...
protected:
  ...
  PQKNodalBasisLocalView<GV, degHess> uDHlocalView_;
  MAMixedHessianTree<GV, deg, degHess> uDHTree_;
  Tree tree_;
};

template<typename GV, int deg, int degHess>
class MAMixedHessianTree: public TypeTree::PowerNode<PQKNodalBasisLeafNode<GV, degHess>,
    GV::dimension * GV::dimension> {

  friend class MAMixedBasisLocalView<GV, deg, degHess> ;

  MAMixedHessianTree(PQKNodalBasisLeafNode<GV, degHess> & leafNode) :
      TypeTree::PowerNode<PQKNodalBasisLeafNode<GV, degHess>,
          GV::dimension * GV::dimension>(leafNode,
          false /* don't make copies */) {
  }
};

//todo erben ist falsch
template<typename GV, int deg, int degHess>
class MAMixedBasisTree: public TypeTree::CompositeNode<
    PQKNodalBasisLeafNode<GV, deg>, MAMixedHessianTree<GV, deg, degHess>> {
  friend MAMixedBasisLocalView<GV, deg, degHess> ;

  typedef TypeTree::CompositeNode<PQKNodalBasisLeafNode<GV, deg>,
      MAMixedHessianTree<GV, deg, degHess> > Base;
  MAMixedBasisTree(PQKNodalBasisLeafNode<GV, deg> & uleafNode,
      const MAMixedHessianTree<GV, deg, degHess> & hessianTree) :
